...
def Attack(): 
    ...
    if TURN_VALUE == 0:
        if random.randint(0,100) > CRIT_CHANCE : #Player chooses Attack and doesn't get the 15% chance to proc a critical hit
            DAMAGE = int((random.randint(10,20) * PLAYER_BUFF_VALUE) *ENEMY_DEFEND_VALUE)
            ENEMY_HEALTH -= DAMAGE
            print(f"You've attacked! The enemy's health is now "+ str(ENEMY_HEALTH)+ ". Strategize for the next turn.")
            print("")
            ...
            ENEMY_HEALTH -= DAMAGE
            print(f"CRITICAL HIT! The enemy's health is now "+ str(ENEMY_HEALTH)+ ". Captialize on the lead you got just now!")
            print("")
        PLAYER_BUFF_VALUE = 1
        ENEMY_DEFEND_VALUE = 1
    elif TURN_VALUE == 1: 
        if CRIT_CHANCE < random.randint(0,100): #Enemy chooses Attack and doesn't get the 15% chance to proc a critical hit
            ...
            print(f"The enemy attacked! Your health is now "+ str(PLAYER_HEALTH)+ ". Strategize for the next turn and get your payback.")
            print("")
            ...
            PLAYER_HEALTH -= DAMAGE
            print(f"WARNING! CRITICAL DAMAGE HAS BEEN TAKEN! Your health is now "+ str(PLAYER_HEALTH)+ ". The situation is looking dire now.")
            print("")
        ENEMY_BUFF_VALUE = 1
...
def Heal():
    ...
    if TURN_VALUE == 0: # Player chooses to heal, affecting their current hp within the specified range.
        ...
        print(f"You look at your phone and see a notification that states that your current HP is "+str(PLAYER_HEALTH)+ "HP.")
        print("")
        PLAYER_HEALTH += HEAL_AMOUNT
        print(f"You take your trusty snack out that never fails you, healing you back to "+str(PLAYER_HEALTH)+ "HP.")
        print("")
    elif TURN_VALUE == 1: # Enemy chooses to heal, affecting their current hp within the specified range.
        HEAL_AMOUNT = random.randint(5, 15)
        print(f"The enemy laughs at you and states that their current HP is "+str(ENEMY_HEALTH)+ "HP.")
        print("")
        ENEMY_HEALTH += HEAL_AMOUNT
        print(f"The enemy eats a snack and states that their current HP is now "+str(ENEMY_HEALTH)+ "HP.")
        print("")
    TURN_VALUE = 1 - TURN_VALUE # This switches turns
    ...
    while PLAYER_HEALTH > 0 and ENEMY_HEALTH > 0:
        if TURN_VALUE == 0:
            ...
            if SELECTION == "Attack":
                ...
                print (f"Your current HP is "+str(PLAYER_HEALTH)+". "+" The enemy's current HP is "+str(ENEMY_HEALTH)+".")
                print("")
            else:
